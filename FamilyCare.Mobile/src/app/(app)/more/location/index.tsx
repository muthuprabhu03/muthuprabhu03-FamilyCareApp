  ...
  const handleToggleSharing = async (value: boolean) => {
    ...
    try {
      if (value) {
        ...
        await locationSharingService.requestAndSendCurrentLocation();
      }
      const res = await locationSharingService.toggleSharing(value);
      setIsSharing(res.isSharing);
      Alert.alert(
        res.isSharing ? 'Location Sharing Enabled' : 'Location Sharing Disabled',
        res.isSharing
          ? 'Your location is now shared with authorized family members in real-time.'
          : 'Your location sharing has been turned OFF. Family members can no longer see your position.'
      );
      loadData();
    } catch (error: any) {
      setIsSharing(!value);
